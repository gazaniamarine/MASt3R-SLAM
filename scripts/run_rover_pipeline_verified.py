...
Same five stages as `run_rover_pipeline.py` (`frontend -> fuse -> locate ->
goal -> plan`), with one difference: `locate` calls
`fact3r-map/scripts/resolve_semantic_goal_verified.py` instead of
`resolve_semantic_goal.py`. That script does SigLIP shortlist -> Qwen3-VL
photo verification -> per-map memory recall for repeat/"go back to X"
queries -> a single-marker confirmation render, instead of pure unverified
SigLIP retrieval. Its output is the same `fact3r-semantic-goal-request`
schema, so `goal` and `plan` -- and everything else below -- are byte-for-
byte identical to `run_rover_pipeline.py`.

This is a separate file, not an edit to `run_rover_pipeline.py`, so the
working pipeline stays untouched.

    python3 scripts/run_rover_pipeline_verified.py \\
        --run logs/rover/pipeline/mpl_20260826_verified \\
    ...
    locate     SAM2                 SigLIP text query -> Qwen3-VL verifies
                                    against photo evidence, checking memory
                                    first -> the winning entity's footprint
...
split across the boundary for exactly that reason: the SigLIP encoder and
Qwen3-VL are only in SAM2, and `HM3DMap` -- whose clearance field and
connected components decide whether a goal is reachable -- imports scipy,
which SAM2 does not have.
...
def build_stages(args, run_dir):
    ...
    confirmation = locate_dir / f"{slug}_goal_request.confirmation.png"
    ...
    locate = Stage(
        ...
        args.sam2_env,
        [
            "python3", REPO / "fact3r-map" / "scripts" / "resolve_semantic_goal_verified.py",
            "--map", stem,
            "--query", args.query,
            "--output", request,
            "--confirmation-image", confirmation,
            "--siglip-top-k", args.siglip_top_k,
            "--vlm-model", args.vlm_model,
            "--min-vlm-confidence", args.min_vlm_confidence,
            "--max-entities", args.max_entities,
            "--device", args.device,
        ]
        + (["--no-memory"] if args.no_memory else [])
        + (["--allow-unverified-fallback"] if args.allow_unverified_fallback else []),
        inputs=[pathlib.Path(f"{stem}_semantic.json"),
                pathlib.Path(f"{stem}_semantic_bev.npz")],
        outputs=[request, confirmation],
    )
    ...
    return {"frontend": frontend, "fuse": fuse, "locate": locate,
        ...
        "stem": stem, "request": request, "confirmation": confirmation,
        "goal": goal, "plans_dir": plans_dir}
...
def main():
    ...
    # locate stage: resolve_semantic_goal_verified.py's most operationally
    # relevant knobs. Anything finer (evidence-views, memory-min-similarity,
    # cache-directory, ...) stays at that script's own defaults here -- run
    # it directly for finer control, same as resolve_semantic_goal.py's own
    # knobs were never all surfaced by run_rover_pipeline.py either.
    ap.add_argument("--siglip-top-k", type=int, default=8,
                    help="shortlist size handed to Qwen for verification")
    ap.add_argument("--vlm-model", default="Qwen/Qwen3-VL-8B-Instruct",
                    help="needs ~18-20 GB free VRAM; pass "
                         "Qwen/Qwen3-VL-2B-Instruct for a busy GPU")
    ap.add_argument("--min-vlm-confidence", type=float, default=0.75)
    ap.add_argument("--max-entities", type=int, default=3)
    ap.add_argument("--no-memory", action="store_true",
                    help="skip the per-map memory check/write entirely")
    ap.add_argument("--allow-unverified-fallback", action="store_true",
                    help="on total VLM rejection, fall back to the top "
                         "SigLIP-only candidate tagged vlm_verified=false "
                         "instead of stopping the locate stage")
