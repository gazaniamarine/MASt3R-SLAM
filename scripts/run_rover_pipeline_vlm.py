...
`fact3r-map/scripts/resolve_semantic_goal_vlm.py` instead of
`resolve_semantic_goal.py` or `resolve_semantic_goal_verified.py`. That
script does SigLIP shortlist -> Qwen3-VL *points at a pixel* on the rendered
top-down semantic BEV (numbered markers), instead of unverified SigLIP
retrieval or per-candidate photo verification -- Qwen identifies the target
by pointing at the map, not by reading a name off a photo crop or judging a
crop's likeness to the query. Its output is the same
`fact3r-semantic-goal-request` schema, so `goal` and `plan` -- and everything
else below -- are byte-for-byte identical to `run_rover_pipeline.py` and
`run_rover_pipeline_verified.py`.

This is a separate file, not an edit to either of those, so the working
pipelines stay untouched.

    python3 scripts/run_rover_pipeline_vlm.py \\
        --run logs/rover/pipeline/mpl_20260826_vlm \\
    ...
    locate     SAM2                 SigLIP shortlist -> Qwen3-VL points at a
                                    pixel on the rendered semantic BEV
    goal       mast3r-slam          that footprint -> a reachable world (y, x)
    plan       mast3r-slam          SafeDiffuser + DSTT to that goal, via an
                                    A* centerline (the only route source)
...
def build_stages(args, run_dir):
    ...
    rendered_map = locate_dir / f"{slug}_goal_request.map.png"
    ...
    locate = Stage(
        ...
        args.sam2_env,
        [
            "python3", REPO / "fact3r-map" / "scripts" / "resolve_semantic_goal_vlm.py",
            "--map", stem,
            "--query", args.query,
            "--output", request,
            "--rendered-map", rendered_map,
            "--siglip-top-k", args.siglip_top_k,
            "--siglip-device", args.device,
            "--vlm-model", args.vlm_model,
        ],
        inputs=[pathlib.Path(f"{stem}_semantic.json"),
                pathlib.Path(f"{stem}_semantic_bev.npz")],
        outputs=[request, rendered_map],
    )
    ...
    return {"frontend": frontend, "fuse": fuse, "locate": locate,
        ...
        "stem": stem, "request": request, "rendered_map": rendered_map,
...
def main():
    ...
    # locate stage: resolve_semantic_goal_vlm.py's most operationally
    # relevant knobs. Anything finer (top-views, min-cells, max-markers,
    # fallback-search-radius-cells, ...) stays at that script's own defaults
    # here -- run it directly for finer control, same as resolve_semantic_
    # goal.py's own knobs were never all surfaced by run_rover_pipeline.py
    # either.
    ap.add_argument("--siglip-top-k", type=int, default=8,
                    help="shortlist size rendered as numbered markers for "
                         "Qwen to point at")
    ap.add_argument("--vlm-model", default="Qwen/Qwen3-VL-2B-Instruct",
                    help="the checkpoint meant to also serve the NavDP "
                         "integration later; keep this the 2B model rather "
                         "than swapping in a bigger one")
