...
try:
    ...
    class Tap:
        """Stub so `class Parser(Tap)` still defines without typed-argument-parser."""
        def __init__(self, *args, **kwargs):
            raise ImportError(
                'typed-argument-parser (tap) is required for the maze2d CLI parser; '
                'the HM3D scripts use argparse instead.')
import pdb
