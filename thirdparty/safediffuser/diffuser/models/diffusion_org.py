...
class GaussianDiffusion(nn.Module):
    ...
    def stt_potential(self, pos, gamma_L, gamma_U):
        """
        pos      : (B,H,2)
        gamma_L  : (H,2)
        gamma_U  : (H,2)

        returns scalar potential
        """

        if gamma_L.dim() == 2:
            gamma_L = gamma_L.unsqueeze(0)
            gamma_U = gamma_U.unsqueeze(0)

        c = (gamma_L + gamma_U) / 2.0
        r = (gamma_U - gamma_L) / 2.0

        d2 = torch.sum((pos - c)**2, dim=-1)

        r2 = torch.sum(r**2, dim=-1)

        eps = 1e-6

        phi = -torch.log(torch.clamp(r2 - d2, min=eps))

        return phi.mean()

    def stt_controller(self, pos, gamma_L, gamma_U, K=1.0):
        """
        Applies the STT closed‑form controller to the position part of a trajectory.
        
        Args:
            pos:      (batch, horizon, 2)   positions (y, x) in normalized [-1,1]
            gamma_L:  (horizon, 2)          lower tube bounds (y, x) normalized
            gamma_U:  (horizon, 2)          upper tube bounds (y, x) normalized
            K:        control gain (positive)
        Returns:
            pos_corrected: (batch, horizon, 2) positions after STT correction
        """
        # Add batch dimension to tube bounds if needed
        if gamma_L.dim() == 2:
            gamma_L = gamma_L.unsqueeze(0)   # (1, horizon, 2)
            gamma_U = gamma_U.unsqueeze(0)
        
        # Compute center and half‑width (radius) per dimension
        c = (gamma_L + gamma_U) / 2.0          # (1, horizon, 2)
        r = (gamma_U - gamma_L) / 2.0          # (1, horizon, 2)
        
        # Normalized error e = (pos - c) / r   (elementwise per dimension)
        e = (pos - c) / (r + 1e-8)
        # Clamp to avoid log(0) – as in the paper
        e = torch.clamp(e, -0.99, 0.99)
        
        # Transformed error ε = log((1+e)/(1-e))
        eps = torch.log((1 + e) / (1 - e))
        
        # STT control: Δpos = -K * (4/(r*(1-e²))) * eps
        # The factor 4/(r*(1-e²)) comes from ξ in the paper
        gain = 4.0 / (r * (1 - e**2))
        delta = -K * gain * eps
        
        # Corrected positions
        pos_corrected = pos + delta
        return pos_corrected

    def compute_stt_tube(self, start_norm, goal_norm, horizon, t_c):
        """
        Computes lower and upper tube bounds for each time step.
        start_norm, goal_norm: (2,) tensors in normalized [-1,1] (y, x)
        Returns:
            gamma_L, gamma_U: (horizon, 2) tensors
        """
        device = self.betas.device
        t_phys = torch.linspace(0, t_c, horizon, device=device).view(-1, 1)  # (horizon,1)
        # Cubic Bézier center
        s = 3 * (t_phys/t_c)**2 - 2 * (t_phys/t_c)**3
        c = start_norm.unsqueeze(0) + s * (goal_norm.unsqueeze(0) - start_norm.unsqueeze(0))  # (horizon,2)

        # Deform center away from red obstacles (same repulsion as before)
        cy, cx = c[:, 0:1], c[:, 1:2]
        # Obstacle 1 (ellipse)
        ...
        off_y1 = 2*(5.0-0.5 - self.norm_mins[0])/(self.norm_maxs[0]-self.norm_mins[0]) - 1
        b1 = ((cy - off_y1)/yr1)**2 + ((cx - off_x1)/xr1)**2 - 1
        grad1 = torch.cat([2*(cy-off_y1)/yr1**2, 2*(cx-off_x1)/xr1**2], dim=-1)
        push1 = 0.15 * torch.clamp(0.2 - b1, min=0.0) * grad1 / (grad1.norm(dim=-1, keepdim=True)+1e-8)

        # Obstacle 2 (superellipse)
        xr2 = 2*1/(self.norm_maxs[1]-self.norm_mins[1])
        yr2 = 2*1/(self.norm_maxs[0]-self.norm_mins[0])
        off_x2 = 2*(5.3-0.5 - self.norm_mins[1])/(self.norm_maxs[1]-self.norm_mins[1]) - 1
        off_y2 = 2*(2.0-0.5 - self.norm_mins[0])/(self.norm_maxs[0]-self.norm_mins[0]) - 1
        b2 = ((cy - off_y2)/yr2)**4 + ((cx - off_x2)/xr2)**4 - 1
        grad2 = torch.cat([4*((cy-off_y2)/yr2)**3/yr2, 4*((cx-off_x2)/xr2)**3/xr2], dim=-1)
        push2 = 0.15 * torch.clamp(0.2 - b2, min=0.0) * grad2 / (grad2.norm(dim=-1, keepdim=True)+1e-8)

        c = c + push1 + push2

        # Compute clearance (distance to red obstacles)
        dist1 = torch.sqrt(((cy - off_y1)/yr1)**2 + ((cx - off_x1)/xr1)**2)
        dist2 = torch.sqrt(((cy - off_y2)/yr2)**2 + ((cx - off_x2)/xr2)**2)
        clearance_obs = torch.min(dist1, dist2)   # (horizon,1)

        # Wall clearance using maze grid (signed distance)
        y_real = (cy.squeeze(-1) + 1) * 0.5 * (self.norm_maxs[0] - self.norm_mins[0]) + self.norm_mins[0]
        x_real = (cx.squeeze(-1) + 1) * 0.5 * (self.norm_maxs[1] - self.norm_mins[1]) + self.norm_mins[1]
        maze_arr = torch.tensor([
                                [10,10,10,10,10,10,10,10,10,10,10,10],
                                [10,11,11,11,11,10,11,11,11,11,11,10],
                                [10,11,10,10,11,10,11,10,11,10,11,10],
                                [10,11,11,11,11,11,11,10,11,11,11,10],
                                [10,11,10,10,10,10,11,10,10,10,11,10],
                                [10,11,11,10,11,10,11,11,11,11,11,10],
                                [10,10,11,10,11,10,11,10,11,10,10,10],
                                [10,11,11,10,11,11,11,10,11,12,11,10],
                                [10,10,10,10,10,10,10,10,10,10,10,10],
                            ], device=device)
        row = torch.clamp(torch.floor(y_real).long(), 0, maze_arr.shape[0]-1)
        col = torch.clamp(torch.floor(x_real).long(), 0, maze_arr.shape[1]-1)
        is_wall = maze_arr[row, col] == 10
        # Approximate distance to nearest free cell: small inside wall, large elsewhere
        wall_dist = torch.where(is_wall, torch.tensor(0.05, device=device), torch.tensor(0.5, device=device)).unsqueeze(-1)

        clearance = torch.min(clearance_obs, wall_dist)   # (horizon,1)

        # Radius: small near obstacles/walls, larger in free space
        r_min, r_max = 0.04, 0.15
        r = r_min + (r_max - r_min) * torch.tanh(clearance / 0.1)   # (horizon,1)

        # Tube bounds
        gamma_L = c - r
        gamma_U = c + r
        # Clip to valid range and enforce L < U
        gamma_L = torch.clamp(gamma_L, -1.0, 1.0)
        gamma_U = torch.clamp(gamma_U, -1.0, 1.0)
        gamma_L = torch.min(gamma_L, gamma_U - 1e-6)
        gamma_U = torch.max(gamma_U, gamma_L + 1e-6)
    ...
    def p_sample(self, x, cond, t):
        ...
        # x = self.Shield(x, xp1)
        # x = self.GD(x, xp1)
        # Apply STT controller to positions
        # Apply STT controller to positions
        if hasattr(self, 'gamma_L') and hasattr(self, 'gamma_U'):
            pos = xp1[:, :, 2:4]
            pos_corrected = self.stt_controller(pos, self.gamma_L, self.gamma_U, K=1.0)
    ...
    def p_sample_loop(self, shape, cond, verbose=True, return_diffusion=False):
        ...
        # ---------- fixed STT tube: compute once ----------
        # cond is a dict: key = time index, value = tensor (batch, state_dim)
        # For Maze2D, state_dim = 4 (y, x, vy, vx) – positions are at indices 2 and 3
        start_norm = cond[0][0, 2:4]          # (y, x) of first batch element
        goal_norm = cond[self.horizon - 1][0, 2:4]   # (y, x)
        t_c = 10.0   # prescribed time (same as in training)
        horizon = shape[1]
        gamma_L, gamma_U = self.compute_stt_tube(start_norm, goal_norm, horizon, t_c)
        self.gamma_L = gamma_L   # (horizon,2)
        self.gamma_U = gamma_U
        
#         start = cond[0][:, 0:2]          # normalized observation [y,x]
#         goal = cond[self.horizon - 1][:, 0:2]  # initial nominal final position

#         self.stt_center, self.stt_radius = self.stt_build_fixed_tube(
#             start=start,
#             goal=goal,
#             H=shape[1],
# )
