...
Fact3R now has validated keyframe, lifted-proposal, entity, and semantic-fact contracts; a read-only keyframe adapter; a finalized-keyframe disk export; official and Transformers SAM2 automatic mask generation; geometry-aware and image-only mask filtering; mask-to-world lifting; alignment and RGB association visualization; saved-proposal streaming; a dense one-second HM3D segmentation diagnostic with adjacent-mask stability statistics; re-anchored official-SAM2 video tracklets; spatial proposal-entity gating; reusable multi-cue pairwise costs with an optional IoU-weighted temporal cue; exact Hungarian assignment; balanced log-domain Sinkhorn transport; projected entity visibility; strict-support visibility-conditioned unbalanced transport with directional birth/miss residuals; delayed entity creation from accumulated tracklet, residual, and 3D-consistency evidence; batched masked-context SigLIP2 encoding; retrospective observation-to-entity resolution; confirmed-only entity retrieval; contrastive confounder margins; quality-weighted multi-view acceptance; explicit open-set rejection; and HTML/GIF/contact-sheet query galleries containing all indexed views of accepted entities. Persistent confirmation/inactive lifecycle behavior beyond birth, confidence-gated memory, structured fact extraction, intermediate-frame semantic histories, BEV planning, and return navigation still need to be implemented.
...
| Milestone 5 — Long-horizon retrieval and return | In progress | Implemented confirmed-only free-text entity retrieval with prompt ensembles, explicit confounders, mask/UOT quality weights, multi-view support, open-set rejection, and a gallery/GIF showing every indexed frame for accepted entities. Still missing intermediate 30-FPS propagation, query-time grounding verification, calibrated real-scene thresholds, BEV occupancy/traversability, topology, goal-pose generation, structured query parsing, graph matching, navigation, and evaluation. |
